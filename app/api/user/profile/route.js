...
export async function PUT(request) {
  ...
  try {
    ...
    if (body.action === 'change_password') {
      ...
      if (!isValid) return NextResponse.json({ message: 'Mật khẩu hiện tại không đúng' }, { status: 400 });
    ...
    if (username) {
      const [existing] = await pool.query('SELECT id FROM users WHERE username = ? AND id != ?', [username, session.userId]);
      if (existing.length > 0) return NextResponse.json({ message: 'Tên đăng nhập đã được sử dụng' }, { status: 400 });
    }
    if (email) {
      const [existing] = await pool.query('SELECT id FROM users WHERE email = ? AND id != ?', [email, session.userId]);
      if (existing.length > 0) return NextResponse.json({ message: 'Email đã được sử dụng' }, { status: 400 });
    }
