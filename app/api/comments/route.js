...
export async function GET(request) {
  ...
  try {
    ...
    let query = `SELECT c.id, c.movie_slug, c.content, c.rating, c.created_at, u.username
                 FROM comments c JOIN users u ON c.user_id = u.id WHERE c.status = 'visible'`;
    const params = [];
...
export async function POST(request) {
  ...
  try {
    await ensureDatabaseSchema();
    const { movie_slug, content, rating } = await request.json();

    if (!movie_slug || !content?.trim()) {
      return NextResponse.json({ message: 'Yêu cầu movie_slug và nội dung.' }, { status: 400 });
    }

    const [result] = await pool.query(
      "INSERT INTO comments (user_id, movie_slug, content, rating, status) VALUES (?, ?, ?, ?, 'visible')",
      [session.userId, movie_slug, content.trim(), rating ? Number(rating) : null]
    );

    return NextResponse.json({ message: 'Đã tạo bình luận', id: result.insertId });
  } catch (error) {
    console.error('Create comment error:', error);
    return NextResponse.json({ message: 'Lỗi tạo bình luận' }, { status: 500 });
  }
}
...
export async function PUT(request) {
  ...
  try {
    ...
    if (rows.length === 0) return NextResponse.json({ message: 'Bình luận không tồn tại' }, { status: 404 });
    ...
    if (fields.length === 0) return NextResponse.json({ message: 'Không có gì cập nhật' }, { status: 400 });
