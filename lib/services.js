...
/**
 * Mix VIP movies into a list of free movies with 7:2 ratio (7 free, 2 VIP per 9)
 * VIP movies are placed at positions 4 and 8 (0-indexed: 3, 7) in each group of 9
 */
export function mixMoviesWithVip(freeMovies, vipMovies, total = 12) {
  if (!vipMovies || vipMovies.length === 0) return freeMovies.slice(0, total);
  if (!freeMovies || freeMovies.length === 0) return vipMovies.slice(0, total);
  ...
  for (let i = 0; i < total; i++) {
    const posInGroup = i % 9; // position within each group of 9
    const isVipSlot = posInGroup === 3 || posInGroup === 7; // positions 4 and 8

    if (isVipSlot && vipIdx < vipMovies.length) {
      const movie = vipMovies[vipIdx++];
      if (!seen.has(movie.slug)) {
        seen.add(movie.slug);
        result.push({ ...movie, isVip: true });
        continue;
      }
    }

    // Fill with free movie
    while (freeIdx < freeMovies.length && seen.has(freeMovies[freeIdx].slug)) {
      freeIdx++;
    }
    if (freeIdx < freeMovies.length) {
      const movie = freeMovies[freeIdx++];
      seen.add(movie.slug);
      result.push(movie);
    }
  }
...
export const getMoviesBySection = unstable_cache(
    async (section) => {
        ...
        return data.filter(m => m).map(withVipMeta); // Filter nulls
    } catch (e) {
...
/**
 * Fetch a pool of VIP movies (action + horror + netflix keyword)
 */
export const getVipMoviePool = unstable_cache(
  async () => {
    try {
      const [action, horror] = await Promise.all([
        axiosInstance.get(`${API_BASE}/v1/api/the-loai/hanh-dong?limit=12`).catch(() => ({ data: { data: { items: [] } } })),
        axiosInstance.get(`${API_BASE}/v1/api/the-loai/kinh-di?limit=12`).catch(() => ({ data: { data: { items: [] } } })),
      ]);
      ...
      return combined
        ...
        .map(m => ({ ...formatMovie(m), isVip: true }));
    } catch (e) {
      return [];
    }
  },
  ['vip-movie-pool'],
  { revalidate: 1800, tags: ['movies'] }
);
        ...
        if (response.data?.status === true || response.data?.status === 'success') {
            if (response.data?.data) {
                return {
                    ...
                    movies: (response.data.data.items || []).map(formatMovie).map(withVipMeta),
