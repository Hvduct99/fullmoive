...
export const getMoviesBySection = unstable_cache(
    async (section) => {
        ...
        try {
            if (section === 'netflix') {
                // Complex logic: Search "Netflix" + multiple genres 2025/2026
                const [netflixSearch, action, horror, romance, series, single] = await Promise.all([
                    ...
                    axiosInstance.get(`${API_BASE}/v1/api/the-loai/hanh-dong?limit=64`).catch(() => ({ data: { data: { items: [] } } })),
                    axiosInstance.get(`${API_BASE}/v1/api/the-loai/kinh-di?limit=64`).catch(() => ({ data: { data: { items: [] } } })),
                    axiosInstance.get(`${API_BASE}/v1/api/the-loai/tinh-cam?limit=64`).catch(() => ({ data: { data: { items: [] } } })),
                    axiosInstance.get(`${API_BASE}/v1/api/danh-sach/phim-bo?limit=64`).catch(() => ({ data: { data: { items: [] } } })),
                    axiosInstance.get(`${API_BASE}/v1/api/danh-sach/phim-le?limit=64`).catch(() => ({ data: { data: { items: [] } } }))
                ]);
                ...
                data = combined.filter(m => {
                    ...
                    seen.add(m.slug);
                    return !duplicate;
                }).sort(() => 0.5 - Math.random()).slice(0, 48).map(formatMovie); // Limit bumped to 48

            } else if (section === 'featured') {
                let sliderMovies = [];
                try {
                    // Get New Updated movies which are usually hot
                    const newMoviesFn = await axiosInstance.get(`${API_BASE}/danh-sach/phim-moi-cap-nhat?page=1`);
                    if (newMoviesFn.data?.items) {
                        sliderMovies = newMoviesFn.data.items.slice(0, 6);
                    }
                } catch (e) { }

                // Fetch details for better images
                if (sliderMovies.length > 0) {
                     const detailPromises = sliderMovies.map(m => axiosInstance.get(`${API_BASE}/phim/${m.slug}`));
            ...
            const response = await axiosInstance.get(`${API_BASE}/v1/api/danh-sach/phim-chieu-rap?limit=18`);
            ...
            const [action, horror] = await Promise.all([
                 axiosInstance.get(`${API_BASE}/v1/api/the-loai/hanh-dong?limit=24`),
                 axiosInstance.get(`${API_BASE}/v1/api/the-loai/kinh-di?limit=24`)
            ]);
            ...
            // Deduplicate and Randomize
                 ...
                 data = movies.filter(m => {
                     ...
                     seen.add(m.slug);
                     return !duplicate;
                 }).sort(() => 0.5 - Math.random()).slice(0, 18).map(formatMovie);

        } else if (section === 'section2') {
             const anime = await axiosInstance.get(`${API_BASE}/v1/api/danh-sach/hoat-hinh?limit=24`);
             const romance = await axiosInstance.get(`${API_BASE}/v1/api/the-loai/tinh-cam?limit=24`);
             let movies = [];
...
{ revalidate: 3600, tags: ['movies'] } // Update every 1 hour
);
