            ...
            if (section === 'netflix') {
                ...
                try {
                    const [sciFi, horror] = await Promise.all([
                        axiosInstance.get(`${API_BASE}/v1/api/the-loai/vien-tuong?limit=24`).catch(() => ({ data: { data: { items: [] } } })),
                        axiosInstance.get(`${API_BASE}/v1/api/the-loai/kinh-di?limit=24`).catch(() => ({ data: { data: { items: [] } } }))
                    ]);
                    ...
                    sliderMovies = combined.filter(m => {
                        ...
                        seen.add(m.slug);
                        return true;
                    }).sort((a, b) => (b.year || 0) - (a.year || 0)).slice(0, 8);
                } catch (e) {
